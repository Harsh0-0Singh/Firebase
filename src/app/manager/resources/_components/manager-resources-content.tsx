...
import { updateResourceRequest } from '@/app/actions/resourceRequests';

interface ManagerResourcesContentProps {
    initialRequests: ResourceRequest[];
    employees: Employee[];
}

export function ManagerResourcesContent({ initialRequests, employees }: ManagerResourcesContentProps) {
  ...
  const handleApproval = async () => {
    ...
    const result = await updateResourceRequest({
  ...
  const handleReject = async (requestId: string) => {
    const result = await updateResourceRequest({ requestId, status: 'Rejected' });
    if(result.success) {
        const updatedRequests = requests.map(r => 
  ...
  const handleComplete = async (requestId: string) => {
    const result = await updateResourceRequest({ requestId, status: 'Completed' });
    if (result.success) {
        const updatedRequests = requests.map(r => 
